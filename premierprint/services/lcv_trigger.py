...
def on_cancel(doc, method):
	"""PI bekor qilganda LCVlarni bekor qilish"""

	lcv_names = frappe.db.get_all(
		"Landed Cost Voucher",
		filters={"custom_purchase_invoice": doc.name, "docstatus": 1},
		pluck="name"
	)

	for name in lcv_names:
		try:
			frappe.get_doc("Landed Cost Voucher", name).cancel()
		except Exception as e:
			frappe.log_error(str(e), f"LCV Cancel: {name}")

	if lcv_names:
		frappe.db.commit()
		frappe.msgprint(f"{len(lcv_names)} ta LCV bekor qilindi")
...
def _cancel_linked_lcvs(doc, method):
	...
	FIXED: Removed custom_lcv_type reference (field doesn't exist)
	"""
	...
	linked_lcvs = frappe.db.sql("""
		...
		WHERE custom_purchase_invoice = %s
		AND docstatus = 1
	""", doc.name, pluck='name')
	...
	for lcv_name in linked_lcvs:
		try:
			...
			errors.append(f"{lcv_name}: {str(e)}")
			frappe.log_error(
				message=frappe.get_traceback(),
				title=f"LCV Cancellation Failed: {lcv_name}"
			)
	...
	if cancelled_count > 0:
		frappe.msgprint(
			_("{0} ta LCV muvaffaqiyatli bekor qilindi").format(cancelled_count),
			indicator="green",
			alert=True
		)
	...
	if errors:
		frappe.msgprint(
			_("Quyidagi LCVlarni bekor qilishda xatolik:<br>{0}").format("<br>".join(errors)),
			indicator="orange",
			alert=True
		)
